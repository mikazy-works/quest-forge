...
Supabase無料プランは約1週間アクセスがないとプロジェクトが自動休止し、休止中は診断結果の保存・表示が失敗します。
対策として、Vercel Cron が毎日 03:00 UTC に `/api/keepalive` を呼び出し、DBに軽いクエリを送って休止タイマーをリセットします(`vercel.json`)。

- 追加の設定は不要です(Vercelにデプロイすれば自動で有効)
- 任意で環境変数 `CRON_SECRET` を設定すると、cron以外からのキープアライブ呼び出しを401で拒否します
